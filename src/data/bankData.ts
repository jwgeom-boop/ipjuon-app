...
export const COMPLEX_BANK_MAP: ComplexData[] = [
  {
    complex: "스마트 아파트",
    banks: [
      { name: "KB국민은행", rate: "3.45~3.80", type: "1금융" },
      { name: "신한은행", rate: "3.35~3.70", type: "1금융" },
      { name: "새마을금고", rate: "3.90~4.20", type: "2금융" },
    ],
  },
  {
    complex: "행복 아파트",
    banks: [
      { name: "하나은행", rate: "3.55~3.90", type: "1금융" },
      { name: "우리은행", rate: "3.60~3.95", type: "1금융" },
    ],
  },
];

/**
 * Given a complex name, returns filtered banks with complex-specific rates.
 * If no match, returns null (show all banks).
 */
export function getBanksForComplex(complexName: string | undefined | null): { banks1: BankInfo[]; banks2: BankInfo[] } | null {
  ...
  for (const cb of match.banks) {
    ...
    if (!base) continue;
    const entry: BankInfo = { ...base, rate: `연 ${cb.rate}%` };
    if (cb.type === "1금융") banks1.push(entry);
    else banks2.push(entry);
  }

  return { banks1, banks2 };
}
